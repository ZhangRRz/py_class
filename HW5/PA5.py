...
emptyNames = frozenset({'E', 'e', 'Empty', 'empty', 'EMPTY', 'H', 'h',
                        'Hollow', 'hollow', 'HOLLOW', 'O', 'o', 'Open', 'open', 'OPEN', '⚪'})
fullNames = frozenset({'F', 'f', 'Full', 'full', 'FULL', 'Filled', 'filled',
                       'FILLED', 'Fill', 'fill', 'FILL', 'c', 'C', 'Closed', 'closed', 'CLOSED', '⚫'})

piece = dict(ChainMap(dict(zip_longest(list(emptyNames), [
             '⚪']*16)), dict(zip_longest(list(fullNames), ['⚫']*17))))
...
def drop(*moves, player='Auto', toggle=True, **kws):
    ...
    n = kws["numMoves"] if "numMoves" in kws else len(
        moves) if len(moves) else 1

    for l in range(n):
        token = '⚪⚫'[isFilled] if player == 'Auto' else piece[player]
        if len(moves) == 0:
            try:
                moves = [int((y:=reduce(concat, iter(str(transpose((z:=[str(i+1) for i in range(7) if(mul(eq(B[i][0], '  '), i+1))])[rnd(len(z))]))))))]
...
# would get the desired board positions.)
                ...
                if "announce" in kws:
                    print("The board's full, so", token,
