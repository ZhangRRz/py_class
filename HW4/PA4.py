...
isFilled = True
B = (["  "]*6, ["  "]*6, ["  "]*6, ["  "]*6, ["  "]*6, ["  "]*6, ["  "]*6)
piece = {'Filled': '⚫', 'filled': '⚫', 'F': '⚫', 'f': '⚫', '⚫': '⚫', 'Empty': '⚪', 'empty': '⚪', 'E': '⚪',
         'e': '⚪', 'Hollow': '⚪', 'hollow': '⚪', 'H': '⚪', 'h': '⚪', '⚪': '⚪', 'Full': '⚫', 'full': '⚫'}
    ...
    for l in range(n):
        ...
        if len(moves) == 0:
            try:
                ...
                moves = [list(filter(lambda x:B[x][0] == '  ', range(7)))[
                    rnd(len(list(filter(lambda x:B[x][0] == '  ', range(7)))))]+1]
            except:
                if "announce" in kws:
